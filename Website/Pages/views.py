...
def Home(request):
    return render(
        ...
        "urls/HomePage.html",
        {
            "declared": importantMeeting.objects.filter(declared=True).count(),
            "taken": importantMeeting.objects.filter(taken=True).count(),
            "plannedMeeting": importantMeeting.objects.filter(planned=True).count(),
            "comingUp": OfficialHoliday.objects.filter(coming_up=True).count(),
            "taken": OfficialHoliday.objects.filter(taken=True).count(),
            "planned2": OfficialHoliday.objects.filter(planned=True).count(),
            "sickLeave": Vacation.objects.filter(reason="Sick Leave").count(),
            "available": Training.objects.filter(available=True).count(),
            "taken2": Training.objects.filter(taken=True).count(),
            "Males": Employee.objects.filter(gender="Male").count(),
            "Females": Employee.objects.filter(gender="Female").count(),
            "Accepted": Vacation.objects.filter(status=True).count(),
            "Approved": Vacation.objects.all().count(),
        },
    )
...
def VacationForm(request):
    if request.method == "POST":
        ...
        new_vac = Vacation(name=namee, vacStart=startD, vacEnd=endD, reason=reasonD)
